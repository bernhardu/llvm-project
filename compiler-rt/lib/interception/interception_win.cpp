...
#pragma include_alias( "../inc/bddisasm_types.h" , "/home/bernhard/data/entwicklung/2024/llvm-mingw/2024-10-18/llvm-mingw/llvm-project/compiler-rt/lib/interception/bddisasm/inc/bddisasm_types.h" )
...
#  if SANITIZER_WINDOWS_x64
  ...
  switch (*(u16*)(address)) {
    // Cannot overwrite control-instruction. Return 0 to indicate failure.
    case 0x25FF:  // FF 25 XX XX XX XX : jmp [XXXXXXXX]
    case 0xFE25:  // 25 fe XX XX XX : and eax,XXXXXXXX
      return 0;
  }

  /* This needs to return 0 just because of test kUnpatchableCode2 - is there a reason? */
  bool need_fail = false;
  switch (*(u16*)(address)) {
    case 0x00FF:  // FF 00 : INC dword ptr [rax]
    case 0xC300:  // 00 C3 : ADD bl, al
    case 0xF225:  // 25 f2 72 73 74 : AND eax, 0x747372f2
    case 0x0B0F:  // 0F 0B : UD2
      need_fail = true;
      break;
  }

  switch (0x00FFFFFF & *(u32*)address) {
    case 0xFF5EDB:  // DB 5E FF : FISTP dword ptr [esi-0x1], st0
      return 0;
  }
...
#  endif
    ...
    status = NdDecodeWithContext(&instrux, (const ND_UINT8 *)address, 15, &ctx);
    if (ND_SUCCESS(status)) {
      size_t rel_offset_old = 0;
      size_t ret_old = GetInstructionSize_old(address, &rel_offset_old);

      if (instrux.DispOffset != rel_offset_old) {
          u8 *bytes = (u8 *)address;
          ReportError("different rel_offset: old:%x new:%x from %02x %02x %02x %02x %02x %02x %02x %02x\n", (int)rel_offset_old, (int)instrux.DispOffset, bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5], bytes[6], bytes[7]);
      }
      if (instrux.Length != ret_old) {
          u8 *bytes = (u8 *)address;
          ReportError("different size: old:%d new:%d from %02x %02x %02x %02x %02x %02x %02x %02x\n", (int)ret_old, (int)instrux.Length, bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5], bytes[6], bytes[7]);
      }

      if (rel_offset) {
        *rel_offset = instrux.DispOffset;

        // Special handling for some instructions to satisfy tests
#if !SANITIZER_WINDOWS_x64
        switch (*(u16*)(address)) {
          case 0x458B:  // 8B 45 73 : MOV eax, dword ptr [ebp+0x73]
            *rel_offset = 0; /* bddisasm would return 2, but this makes test TestIdentityFunctionPatching(kIdentityCodeWithPrologue) on i686 sad */
            break;
        }
        switch (0x00FFFFFF & *(u32*)address) {
          case 0x24448B:  // 8B 44 24 74 : MOV eax, dword ptr [esp+0x74]
            *rel_offset = 0; /* bddisasm would return 3, but this makes test TestIdentityFunctionPatching(kIdentityCodeWithMov) on i686 sad */
            break;
        }
#endif
      }

      return instrux.Length;
    }
  }
